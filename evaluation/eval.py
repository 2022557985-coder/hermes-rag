...
def evaluate_pipeline(
    ...
    for item in data:
        ...
        relevant_docs = item.get("relevant_doc_ids") or sorted(
            {rid.rsplit("_", 1)[0] for rid in relevant_ids}
        )
        ...
        try:
            ...
            for r in results:
                ...
                if doc not in seen_docs:
                    seen_docs.add(doc)
                    doc_results.append({"chunk_id": doc})
            metrics["doc_hit_rate@1"].append(hit_rate_at_k(doc_results, relevant_docs, 1))
            metrics["doc_hit_rate@3"].append(hit_rate_at_k(doc_results, relevant_docs, 3))
            metrics["doc_hit_rate@5"].append(hit_rate_at_k(doc_results, relevant_docs, 5))
            metrics["doc_mrr"].append(mrr(doc_results, relevant_docs))
            metrics["doc_recall@5"].append(recall_at_k(doc_results, relevant_docs, 5))
    ...
    aggregated = {
        ...
        "doc_hit_rate@1": float(np.mean(metrics["doc_hit_rate@1"])) if metrics["doc_hit_rate@1"] else 0.0,
        "doc_hit_rate@3": float(np.mean(metrics["doc_hit_rate@3"])) if metrics["doc_hit_rate@3"] else 0.0,
        "doc_hit_rate@5": float(np.mean(metrics["doc_hit_rate@5"])) if metrics["doc_hit_rate@5"] else 0.0,
        "doc_mrr": float(np.mean(metrics["doc_mrr"])) if metrics["doc_mrr"] else 0.0,
        "doc_recall@5": float(np.mean(metrics["doc_recall@5"])) if metrics["doc_recall@5"] else 0.0,
...
def main():
    ...
    parser.add_argument("--extra-docs", action="store_true", help="Include all sample documents")
    ...
    output = args.output or str(Path(__file__).parent.parent / "docs" / "eval_results.json")
    results = run_benchmark(output_path=output, variants=variants, extra_docs=args.extra_docs)
